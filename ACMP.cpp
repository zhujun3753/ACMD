...
Camera ReadCamera(const std::string &cam_path)
{
    ...
    if (camera.depth_max<camera.depth_min)
    {
...
int readMask(const std::string file_path, cv::Mat_<uchar> &mask)
{
    cnpy::NpyArray arr = cnpy::npy_load(file_path);
    assert(arr.shape.size()==2);
    float* loaded_data = arr.data<float>();
    mask=cv::Mat(arr.shape[0],arr.shape[1],CV_32F,loaded_data);
    return 0;
}
...
void ACMP::InuputInitialization(const std::string &dense_folder, const Problem &problem)
{
    ...
    if (params.geom_consistency) {
        ...
        std::string suffix = (params.multi_geometry) ? "/depths_geom.dmb" : "/depths.dmb";
...
void ACMP::CudaSpaceInitialization(const std::string &dense_folder, const Problem &problem)
{
    ...
    for (int i = 0; i < num_images; ++i) {
        int rows = images[i].rows; // * 1通道32位浮点型
        int cols = images[i].cols;

        // * 数据准备
        // * 定义类型、分配空间、复制数据
        cudaChannelFormatDesc channelDesc = cudaCreateChannelDesc(32, 0, 0, 0, cudaChannelFormatKindFloat); // * 定义CUDA Array的类型 Signed、Unsigned、Float
